...
# ------------------------------- Figure 3 – Actual & Expected Home Price Growth
# ------------------------------- Panel A - Actual and Expected Home Price Growth - SCE
...
#Crate the plot
Figure_4 <- ggplot() +
  geom_histogram(data = DHS2019, aes(x = exp_home_price_w, y = ..count../DHS_n, fill = "DHS2019"), color = "black", size = 0.2, bins = 40) +
  geom_histogram(data = SCE2019, aes(x = exp_home_price_point_w, y = ..count../SCE_n, fill = "SCE2019"), color = "black", size = 0.2, bins = 40) +
  labs(x = "Expected Home Price Growth (Percent)", y = "Percent", fill = "Dataset") +
  scale_fill_manual(values = c("#2171b5", alpha("#7801ff", 0.5)), labels = c("DHS 2019", "SCE 2019")) +
  scale_y_continuous(labels = scales::percent_format(), expand = expansion(mult = c(0, .0)), 
                     limits = c(0, 0.6), breaks = seq(0, 0.6, by = 0.1)) +
  theme_master_2() +
  theme(legend.position = "bottom", plot.title = element_text(hjust = 0.4),
...
# ------------------------------- Figure A.1 – Actual & Expected Mortgage Rate Growth
# ------------------------------- Panel A: Actual & Expected Mortgage Rate Growth (SCE)
...
# ------------------------------- Figure A.2 – Distribution of Expected Mortgage Rate Growth (DHS & SCE 2019)
# Nuber of rows to calculate fraction for y-axis
DHS2019 <- DHS %>%
  drop_na(exp_home_price_w, exp_1y_mortgage_w) %>%
  filter(year == 2019)
...
Figure_A2 <- ggplot() +
  geom_histogram(data = DHS2019, aes(x = exp_1y_mortgage_w, y = ..count../DHS_n, fill = "DHS2019"), color = "black", size = 0.2, bins = 40) +
  geom_histogram(data = SCE2019, aes(x = exp_mr_growth, y = ..count../SCE_n, fill = "SCE2019"), color = "black", size = 0.2, bins = 40) +
  labs(x = "Expected Mortgage Rate Growth (Percentage Points)", y = "Percent") +
  scale_fill_manual(values = c("#2171b5", alpha("#7801ff", 0.5)), labels = c("DHS 2019", "SCE 2019")) +
  scale_y_continuous(labels = scales::percent_format(), expand = expansion(mult = c(0, .0)), 
                     limits = c(0, 0.6), breaks = seq(0, 0.6, by = 0.1)) +
  theme_master_2() +
...
Figure_3_B
dev.off()
emf("Figure_4.emf", width = 4.725, height = 4.05)
Figure_4
dev.off()
...
Figure_A1_B
dev.off()
emf("Figure_A2.emf", width = 4.725, height = 4.05)
Figure_A2
dev.off()
